...
function gatherKeyOrValue(array) {
  for (let element of array) {
    /*let splitterIndex = element.indexOf(":");
    for (let char of element) {
      if (element.indexOf(char) < splitterIndex) {
        key += char;
      } else if (element.indexOf(char) > splitterIndex) {
        value += char;
      }
    }
    dataObj[key] = value;*/
    let splitter = element.split(":");
    dataObj[splitter[0]] = splitter[1];
  }
}

function checkString(min, max, char, string) {
  ...
  console.log("nb d occurences : ", nbOfOccurences);
  if (nbOfOccurences >= min && nbOfOccurences <= max) {
    return true;
  } else {
    return false;
  }
...
function prepareRule(key) {
  let splitter = key.split(" ");
  let char = splitter[1];
  let boundaries = splitter[0].split("-");
...
console.log("data ->", gatherKeyOrValue(data));
console.log(prepareRule(data[0]));

console.log(dataObj);
console.log(checkString(1, 3, "a", "lala"));
